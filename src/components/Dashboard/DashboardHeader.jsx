  ...
  return <>
    <header className="sticky top-0 z-[100] flex min-h-16 flex-wrap items-center justify-between gap-3 bg-white px-4 py-3 text-slate-700 md:px-6">
      ...
      <div className="flex min-w-0 items-center gap-1.5 text-xs text-slate-900 sm:gap-2">
        ...
        <span className="flex h-7 min-w-7 items-center justify-center rounded-full bg-violet-200 px-2 font-black text-violet-950 shadow-sm">4</span>
        ...
        <div className="relative">
          <button type="button" title="Open AI suggestions" onClick={() => setAiSuggestionsOpen((open) => !open)} className="rounded-full p-2 text-slate-700 transition hover:bg-emerald-100 hover:text-emerald-700"><Menu className="h-4 w-4" /></button>
        ...
        <button type="button" title="Search the app" onClick={() => setSearchOpen(true)} className="hidden rounded-full p-2 text-slate-700 transition hover:bg-sky-100 hover:text-sky-700 sm:inline-flex"><Search className="h-4 w-4" /></button>
        <button type="button" title="Open search" onClick={() => setSearchOpen(true)} className="hidden items-center gap-1 rounded-full bg-amber-200 px-2.5 py-1.5 font-bold text-amber-950 shadow-sm transition hover:bg-amber-300 sm:inline-flex"><Command className="h-3.5 w-3.5" />K</button>
        <button type="button" title={darkMode ? "Switch to light mode" : "Switch to dark mode"} onClick={() => toggleDarkMode(!darkMode)} className="rounded-full p-2 text-slate-700 transition hover:bg-violet-100 hover:text-violet-700">{darkMode ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}</button>
