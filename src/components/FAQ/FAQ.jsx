  ...
  return (
    <main className="min-h-screen bg-slate-50 px-6 pb-12 pt-20 text-slate-900 md:px-10">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mt-4 flex gap-2 overflow-x-auto scroll-smooth pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden [-webkit-mask-image:linear-gradient(to_right,transparent,black_16px,black_calc(100%-16px),transparent)] [mask-image:linear-gradient(to_right,transparent,black_16px,black_calc(100%-16px),transparent)]">{categories.map((itemCategory) => <button key={itemCategory} type="button" onClick={() => setCategory(itemCategory)} className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-semibold transition duration-200 active:scale-95 ${category === itemCategory ? "bg-emerald-600 text-white shadow-sm" : "bg-white text-slate-600 ring-1 ring-slate-200 hover:ring-emerald-300 hover:-translate-y-0.5"}`}>{itemCategory}</button>)}</div>
