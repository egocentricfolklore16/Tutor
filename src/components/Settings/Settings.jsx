...
function Section({ title, description, children, open, onToggle }) {
  return <section className="settings-section"><button type="button" onClick={onToggle} className="flex w-full items-center justify-between gap-4 text-left"><span><h2 className="text-xl font-bold text-slate-900">{title}</h2><p className="mt-1 text-sm text-slate-500">{description}</p></span><ChevronDown className={`shrink-0 text-slate-400 transition-transform ${open ? "rotate-180" : ""}`} size={20} /></button>{open && <div className="mt-7">{children}</div>}</section>;
}

function SelectField({ label, value, onChange, options }) {
  return <label className="block text-sm font-medium text-slate-700">{label}<select value={value} onChange={(event) => onChange(event.target.value)} className="settings-field mt-2 w-full"><option value="">Choose {label.toLowerCase()}</option>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select></label>;
}

function ChoiceRow({ options, value, onChange }) {
  return <div className="grid gap-2 sm:grid-cols-3">{options.map((option) => <button key={option} type="button" onClick={() => onChange(option)} className={`rounded-full px-4 py-3 text-left text-sm font-semibold transition duration-200 active:scale-95 ${value === option ? "bg-emerald-100 text-emerald-800 shadow-sm" : "bg-slate-50 text-slate-600 hover:bg-slate-100 hover:-translate-y-0.5"}`}>{option}</button>)}</div>;
}
...
function Settings() {
  ...
  const save = async () => {
    ...
    const payload = { education_level: settings.studentLevel, settings_subject: settings.subject, current_topic: settings.currentTopic.trim(), curriculum_standard: settings.curriculumStandard, learning_style: settings.learningStyle, knowledge_gaps: settings.knowledgeGaps, socratic_strictness: settings.socraticStrictness, accessibility_needs: settings.accessibilityNeeds, language: settings.language, reduced_motion: settings.reducedMotion, dark_mode: darkMode, ...(selectedFile ? { user_img: userImg } : {}) };
  ...
  return <main className="settings-page min-h-screen px-5 pb-14 pt-20 text-slate-900 md:px-10"><div className="mx-auto max-w-5xl"><header className="mb-7"><h1 className="text-4xl font-black tracking-tight">Settings</h1><p className="mt-2 text-base text-slate-500">Manage your account and preferences</p></header><nav className="settings-tabs mb-6 flex w-full gap-1 overflow-x-auto rounded-full bg-slate-200/80 p-1" aria-label="Settings sections">{tabs.map((tab) => <button key={tab} type="button" onClick={() => setActiveTab(tab)} className={`shrink-0 rounded-full px-5 py-2.5 text-sm font-semibold transition duration-200 active:scale-95 ${activeTab === tab ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:bg-white/50 hover:text-slate-800"}`}>{tab}</button>)}</nav><div className="settings-card">
    {activeTab === "Billing" && <Billing user={currentUser} />}
    {activeTab === "Privacy" && <div className="py-10 text-center"><h2 className="text-xl font-bold text-slate-900">Privacy</h2><p className="mt-2 text-sm text-slate-500">Privacy settings are not available yet.</p></div>}
